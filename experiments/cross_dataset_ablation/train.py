...
def pull_contribs(checkpoint, suffix):
    ...
    print(f"Request URL: https://huggingface.co/henryscheible/{checkpoint}/raw/main/contribs-{suffix}.txt")
...
def test_shapley(eval_checkpoint, eval_loader_func, contrib_checkpoint, suffix):
    REPO = "henryscheible/" + eval_checkpoint
    print(f"=======CHECKPOINT: {eval_checkpoint}==========")
    tokenizer = AutoTokenizer.from_pretrained("bert-base-uncased")
    _, eval_loader = eval_loader_func(tokenizer)
    model = AutoModelForSequenceClassification.from_pretrained(REPO)
    base_acc = evaluate_model(eval_loader, model)

    contribs = pull_contribs(contrib_checkpoint, suffix)
